...
PCB *process_init_PCB(char *fname, unsigned int base)
{
    ...
    strncpy(process.prog_fname, fname, sizeof(process.prog_fname));
    ...
    fprintf(stderr, "[scheduler.c] (process_init_PCB) : Scheduler could not allocate process (fname = %s) in the PCB List.\n", fname);
    return NULL;
}
...
{
    fprintf(stdout, "===========================================\n");
    fprintf(stdout, "           PCB Dump\n");
    fprintf(stdout, "===========================================\n");
    fprintf(stdout, "Index: [ Filename:XXXX, PID:#, BASE:#, PC:#, IR0:#, IR1:#, AC:#, MAR:#, MBR:# ]\n");
    for (int i = 0; i < pcbl->size; ++i)
    {
        if (pcbl->pcb_arr[i].pid != 0)
        {
            fprintf(stdout, "%d: [ Filename:%s,", i, pcbl->pcb_arr[i].prog_fname);
            fprintf(stdout, " PID:%d, BASE:%d,", pcbl->pcb_arr[i].pid, pcbl->pcb_arr[i].reg_state.BASE.reg_val);
            fprintf(stdout, " PC:%d, IR0:%d,", pcbl->pcb_arr[i].reg_state.PC.reg_val, pcbl->pcb_arr[i].reg_state.IR0.reg_val);
            fprintf(stdout, " IR1:%d, AC:%d,", pcbl->pcb_arr[i].reg_state.IR1.reg_val, pcbl->pcb_arr[i].reg_state.AC.reg_val);
            fprintf(stdout, " MAR:%d, MBR:%d ]\n", pcbl->pcb_arr[i].reg_state.MAR.reg_val, pcbl->pcb_arr[i].reg_state.MBR.reg_val);
        }
    }
    fprintf(stdout, "\n");
}
...
{
    fprintf(stdout, "===========================================\n");
    fprintf(stdout, "           ReadyQ Dump. RQ Size: %d\n", rq->size);
    fprintf(stdout, "===========================================\n");
    fprintf(stdout, "Index: Process ID\n");
...
void process_init_idle()
{
    ...
    if (prog_idle == NULL)
    {
        fprintf(stderr, "[scheduler.c] (process_init_idle) : Can't open prog_idle\n");
        return;
    }
...
void process_submit(unsigned int p_base, char *p_fname)
{
    ...
    if (new_proc == NULL)
    {
        fprintf(stderr, "[scheduler.c] (process_submit) : New Process (fname = %s) submit FAILED.\n", p_fname);
        return;
    }
    print_init_spool(new_proc->pid);
    process_insert_readyQ(new_proc);
    fprintf(stdout, "[scheduler.c] (process_submit) : New Process (PID = %d) submitted.\n", new_proc->pid);
}
...
void process_exit(PCB *proc, bool pexit)
{
    ...
    pop_readyQ(rq);
    fprintf(stdout, "[scheduler.c] (process_exit) : Process (PID = %d) exited.\n", exit_pid);
}
...
void scheduler_terminate()
{
    ...
    fprintf(stdout, "[scheduler.c] (scheduler_terminate) : Scheduler shut down complete.\n");
...
void process_execute()
{
    ...
    while (!shut_down)
    {
        ...
        {
            idle = false;
            // fprintf(stdout, "[scheduler.c] (process_execute) : Running Process: %d\n", proc->pid);
        ...
        if (proc_state == -1)
        {
            if (!idle) // (idle -> idle)
            {
                ...
                fprintf(stdout, "[scheduler.c] (process_execute) : Switching Process.\n");
                fprintf(stdout, "    PID in: %d, out: %d\n", in->pid, proc->pid);
                fprintf(stdout, "    PC in: %d, out: %d\n\n", in->reg_state.PC.reg_val, proc->reg_state.PC.reg_val);
            }
        }
        ...
        {
            fprintf(stderr, "[scheduler.c] (process_execute) : Unexpected CPU status: %d. Shutting Down Now.\n", proc_state);
            break;
        }
    ...
    fprintf(stdout, "[scheduler.c] (process_execute) : Scheduler shut down started.\n");
    scheduler_terminate();
}